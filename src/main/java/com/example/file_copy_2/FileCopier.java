...
public class FileCopier {
    ...
    public Task<Void> createCopyVideosTask(String videoPath) {
        return new Task<>() {
            @Override
            protected Void call() throws Exception {
                try (Connection conn = connect()) {
                    ...
                    Path sourceFilePath = Paths.get(videoPath);
                    String videoName = sourceFilePath.getFileName().toString();
                    ...
                    try (InputStream in = Files.newInputStream(sourceFilePath);
                        ...
                        while ((bytesRead = in.read(buffer)) > 0) {
                            ...
                            updateMessage(String.format("Copying %s (%.2f%%)", videoName, progress * 100));
                        }
        ...
        return new Task<>() {
            ...
            protected Void call() throws Exception {
                try(Connection conn = this.connect()){
                    ...
                    updateMessage("Copied : " + succes_ +" from "+photos.size());
                    updateProgress(0, 1.0);
